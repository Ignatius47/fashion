...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".pro-qty-2 input").forEach(input => {
        input.addEventListener("change", function () {
            let cartItemId = this.closest("tr").dataset.id;  // Get item ID
            ...
            .then(data => {
                if (data.success) {
                    this.closest("tr").querySelector(".cart__price").innerText = `$ ${data.new_total}`;
                    updateCartTotal();
                }
            });
        });
    ...
    // Handle item removal
    document.querySelectorAll(".cart__close i").forEach(button => {
        button.addEventListener("click", function () {
            let cartItemId = this.closest("tr").dataset.id;
    ...
    function updateCartTotal() {
        let total = 0;
        document.querySelectorAll(".cart__price").forEach(price => {
